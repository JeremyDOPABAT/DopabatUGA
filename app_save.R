...
library(shinycookie)
library(GAlogger)
ga_set_tracking_id("G-ZSD9TL5TWJ")
...
"
  ...
  observeEvent(c(input$file2),{
    ga_collect_pageview(page = "bib or wos")
    error=NULL
    observeEvent(input$is_wos,{
      ...
      error=tryCatch({
        ...
        ga_collect_event(event_category = "import", event_label = "import bib ",event_action = "tryimport",event_value = dim(reactive_values$data_wos)[1])
      },
      ...
      if(length(error)==1){ 
        if(is.na(error)){
          ga_collect_event(event_category = "Error", event_label = "import bib fail",event_action = "trypageview",event_value = 1)
