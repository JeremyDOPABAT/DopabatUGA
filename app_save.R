                       ...
                       tabItem(tabName = "DB",
                               ...
                               fluidRow(
                                 ...
                                 box(width = 4, title = "Chose the databases you want to use", solidHeader = TRUE, status = "danger",
                                     ...
                                     conditionalPanel('output.show_id_arxiv',selectInput("id_arxiv_selection", "arxiv id column", choices = "", width = "300px"))
...
server <- function(input, output, session) {
  ...
  reactive_values <- reactiveValues(
    ...
    fmts=c("%d/%m/%y","%Y", "%Y-%m", "%m/%d/%y","%d-%m-%Y","%Y-%m-%d"),
    active_source=NULL
  )
  ...
  observeEvent(input$file1, {
    ...
    observeEvent({
      ...
      df <- readr::read.csv(input$file1$datapath,
                            header = input$header,
                            sep = input$sep,
  ...
  observe({ if(reactive_values$valide_csv==TRUE || reactive_values$valide_pdf==TRUE || reactive_values$valide_wos==TRUE){
    ...
    print(" voici deam globale ")
    print(dim(reactive_values$df_global))
  ...
  observeEvent(input$valid_DB, {
    ...
    if(reactive_values$ok_analyse==TRUE){
      ...
      if(input$arxiv==TRUE){
        ...
        if(input$id_arxiv_selection!="none") arxiv_col=input$id_arxiv_selection else arxiv_col=""
        if(input$sup_wos_for_ref==TRUE){
          reactive_values$res_arxiv=extraction_data_api_arxiv(data_pub=reactive_values$df_global,ti_name="titre",au_name="auteur",pas=8,value_same_min_accept=0.95,value_same_min_ask = 0.85,type = input$type,source_name = "source",sep_vector_in_data ="sep",position_vector_in_data = "position_name",id_name =arxiv_col )
        }else{
          reactive_values$res_arxiv=extraction_data_api_arxiv(data_pub=reactive_values$df_global,ti_name="titre",au_name="auteur",pas=8,value_same_min_accept=0.95,value_same_min_ask = 0.85,type = input$type,sep_vector_in_data ="sep",position_vector_in_data = "position_name",id_name = arxiv_col)
        }
      }
