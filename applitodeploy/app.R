                    ...
                    tabItems(
                      ...
                      tabItem(tabName = "import_csv",
                              ...
                              fluidRow(
                                ...
                                box(width = 4, title = "Uploading files", solidHeader = TRUE,status = "danger", #danger=rouge 
                                    ...
                                    #display viewer 
                                    tags$hr(),
                                    radioButtons("lang_csv", "choose languages of keywords",
                                                 choices = c("English" = "en",# mode de visualisation du fichier 
                                                             "French" = "fr"),
                                                 selected = "en"),
                                ...
                                box(width = 5, title = "Uploading bibtex files", solidHeader = TRUE, status = "danger",# le status "danger" permet la couleur rouge 
                                    ...
                                    tags$div(title="the order of the author need to be told, the firstname don't need to be complete abreviation works",radioButtons("position_name_wos", "disposition of name and firstname for author",
                                                                                                                                                                     choices = c("Lastname Firstname" = "2",
                                                                                                                                                                                 "Firstname Lastname" = "1"),
                                                                                                                                                                     selected = "1")),
                                    radioButtons("lang_wos", "choose languages of keywords",
                                                 choices = c("English" = "en",# mode de visualisation du fichier 
                                                             "French" = "fr"),
                                                 selected = "en"),
                      ...
                      tabItem(tabName = "calculinterdisciplinarity",
                              ...
                              fluidPage(
                                ...
                                fluidRow(
                                  column(width = 6,selectInput("select_article", "Select the number of the article for the graph",NULL)
                                  ),column(width = 6,radioButtons("lang_plot", "choose languages of plots",
                                                                  choices = c("English" = "en",# mode de visualisation du fichier 
                                                                              "French" = "fr"),
                                                                  selected = "fr"))),
...
server <- function(input, output, session) {
  ...
  reactive_values <- reactiveValues(
    ...
    pct_ref=c(NULL,NULL,NULL,NULL),#caclule pourcentage exactitude sur les citation  2premier  acuracy ~2 dernier total
    pct_cit=c(NULL,NULL,NULL,NULL),# ""      ""             "      "    "   references 
  ...
  output$text_home <- renderText({
    paste("DOPABAT aims to analyse all the bibliometric data, keywords, domains, citations, references. You can import cvs files or bibtex files to start the analyse. After importation you can choose which database you want to analyse!
           ...
           ",h3("Some examples of what you can do!") 
    )
...
"
  ...
  output$text_interdis_graph <- renderText({
    paste(h4("Help:"), "Here you can see the interdisciplinarity  of each article or the corpus. If you click on a part of the graph you will have data details on a table below.")
  })
  ...
  observeEvent(input$reset_button, {
    showModal(modalDialog(
      title = "The app will reset ",
      "You click on reset button the app will restart know.",
      easyClose = TRUE,
      footer = NULL
    ))
    Sys.sleep(time = 1)
    js$reset()})# permet la fonction reset dans history 
      ...
      if(reactive_values$ok_analyse==TRUE){# si il n'y a pas d'erreur on fait l'analyse de mot clef et domaine 
        ...
        if(is.na(error)){
          ...
          if(is.null(reactive_values$df_global)==TRUE) {# si la table est vide (pas d'autre ficher valider avant, il faut la cree)
            ...
            reactive_values$df_global["lang"]=input$lang_csv
  ...
  observe({ if(reactive_values$valide_csv==TRUE || reactive_values$valide_pdf==TRUE || reactive_values$valide_wos==TRUE){
    ...
    # lematisation
    #browser()
    keywords_lem<-sapply(1:length(keywords_tok),FUN =function(x,l=reactive_values$df_global[["lang"]]) {stemDocument(keywords_tok[[x]],language = l[[x]])})
  ...
  observeEvent(input$valid_wos, {
    ...
    if( reactive_values$ok_analyse==TRUE){
      ...
      if(is.null(reactive_values$df_global)==TRUE) {
        ...
        reactive_values$df_global["id_arxiv"]=NA
        reactive_values$df_global["lang"]=input$lang_wos
  ...
  observeEvent(input$valid_DB, { 
    ...
    if((input$ads==FALSE && reactive_values$pubmed==FALSE && input$arxiv==FALSE && input$lens==FALSE) |is.null(reactive_values$df_global)==TRUE ){
      ...
      showModal(modalDialog(
        ...
        "The processus could be long because we have to respect API rules, please be patient.",
        easyClose = TRUE,
        footer = NULL
      ))
    }
    ...
    if(reactive_values$ok_analyse==TRUE){
      ...
      if((reactive_values$cal_temp$nb_cit+reactive_values$cal_temp$nb_ref)>=1){
        ...
        showModal(modalDialog(
          title = "research ended but no results founded ",
  ...
  # #plot2
    ...
    observeEvent({input$valid_data_research}, {
      ...
      reactive_values$journal_table_ref=read.csv("data/table_journal.csv", sep = ";",header = TRUE,encoding = "Latin-1",stringsAsFactors = FALSE)
      ...
      reactive_values$table_of_traduction=reactive_values$table_categ_gd[,c("OST.Category","OST.Category.english")]
      ind=duplicated(reactive_values$table_of_traduction)
      reactive_values$table_of_traduction=reactive_values$table_of_traduction[which(ind==FALSE),]
      ...
      if(reactive_values$ok_calcule_interdis==TRUE){
        ...
        error=tryCatch({
          withProgress(message = "Dicipline distance calculation",value = 0.90,{       
            res_temp<-global_merge_and_cal_interdis(ads=reactive_values$res_ads,arxiv=reactive_values$res_arxiv,pumed=reactive_values$res_pumed,wos=reactive_values$ref_wos,lens = reactive_values$res_lens,journal_table_ref = reactive_values$journal_table_ref,table_categ_gd = reactive_values$table_categ_gd,type = input$type,table_dist =reactive_values$table_dist,col_journal=c(input$col_journal_ads,input$col_journal_arxiv,input$col_journal_pumed,input$col_journal_wos,input$col_journal_lens),calcul_distance_off=TRUE)    
          })
        },
        error=function(cond){ 
            ...
            if(!is.null(reactive_values$matrice_res_ref$res)){
              ...
              if(input$lang_plot=="en" ){
                ...
                if(reactive_values$plot_in_englis==FALSE){
                  ...
                  reactive_values$plot_in_englis=TRUE
                }
              }else{
                #browser()
                if(reactive_values$plot_in_englis==TRUE){
                  names(reactive_values$matrice_res_ref$res$prop_grande_discipline)=sapply(1:length(names(reactive_values$matrice_res_ref$res$prop_grande_discipline)),FUN = function(x){
                    ind=which(tolower(Unaccent(names(reactive_values$matrice_res_ref$res$prop_grande_discipline)[x]))==tolower(Unaccent(reactive_values$table_of_traduction[,2])))
                    names(reactive_values$matrice_res_ref$res$prop_grande_discipline)[x]=reactive_values$table_of_traduction[ind,1]
                  })
                  reactive_values$plot_in_englis=FALSE
                }
              }
       ...
       #           browser()
                  ...
                  ind=as.numeric(names(which(rowSums(reactive_values$matrice_res_ref$res$prop_grande_discipline[line,])!=0)))#on enleve les ligne égale a 0 et on prend les num de ligne  
            ...
            if(!is.null(reactive_values$matrice_res_cit$res)){
              ...
              observeEvent(input$lang_plot, {
                ...
                if(input$lang_plot=="en" ){
                  ...
                  if(reactive_values$plot_in_englis==TRUE){
                    names(reactive_values$matrice_res_cit$res$prop_grande_discipline)=sapply(1:length(names(reactive_values$matrice_res_cit$res$prop_grande_discipline)),FUN = function(x){
                      ind=which(tolower(Unaccent(names(reactive_values$matrice_res_cit$res$prop_grande_discipline)[x]))==tolower(Unaccent(reactive_values$table_of_traduction[,2])))
                      names(reactive_values$matrice_res_cit$res$prop_grande_discipline)[x]=reactive_values$table_of_traduction[ind,1]
                    })
                    reactive_values$plot_in_englis=FALSE
                  }
                }
              ...
              output$plot_article_cit<-renderPlotly({
               ...
               # if(input$lang_plot=="en") browser()
                error=tryCatch({
                  ...
                  ind=as.numeric(names(which(rowSums(reactive_values$matrice_res_cit$res$prop_grande_discipline[line,])!=0)))# on verifi les ligne null pi on prend les nu&eo de ligne 
                  title_graph=paste0("Main subjects  \n of article ",reactive_values$matrice_res_cit$res$prop[["TITLE"]][line[1]])
                  if(nchar(title_graph)>reactive_values$limit_length_title) title_graph=paste0(substr(title_graph,1,reactive_values$limit_length_title),"...")
                  
                  df <- data.frame(
                    group = Unaccent(names(reactive_values$matrice_res_cit$res$prop_grande_discipline[ind,])),
                    value = colSums(reactive_values$matrice_res_cit$res$prop_grande_discipline[ind,])/sum(reactive_values$matrice_res_cit$res$prop_grande_discipline[line,])
                  )
              ...
              output$plot_total_cit<-renderPlotly({# plot des theme citations 
                ...
                df <- data.frame(
                  ...
                  value = colSums(reactive_values$matrice_res_cit$res$prop_grande_discipline[dim(reactive_values$matrice_res_cit$res$prop_grande_discipline)[1],])/sum(unlist(reactive_values$matrice_res_cit$res$prop_grande_discipline[dim(reactive_values$matrice_res_cit$res$prop_grande_discipline)[1],]))
                )
  ...
  observeEvent(input$remove_cookies, {
    ...
    showModal(modalDialog(
      title = "Cookies are removed",
      "the app succesfuly removed the cookies",
      easyClose = TRUE,
      footer = NULL
    ))
  },ignoreInit = TRUE)
