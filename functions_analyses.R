...
supress_unfit_entry<-function(title_vector,author_vector,max_aut=8){
  ...
  author_vector<-gsub(";",",",author_vector)
  au_sep=strsplit(author_vector,",")
...
#res_temp=supress_unfit_entry(ti_ths,au_ths)
...
#t=compaire_title(ti_ths[first:last],unlist(tps))
...
pumed_get_publi<-function(au_data,ti_data,position_name,pas,value_same_min_ask,value_same_min_accept){
  ...
  for(h in 1:inter){
    ...
    (ti_querry=paste0('%28',"%22",gsub("`",'%60',gsub("[]]","%5D",gsub("[[]","%5B",gsub("[(]","%28",gsub("[)]","%29",gsub("<","%3",gsub(">","%3E",gsub("=","%3D",gsub('[}{$]',"",gsub("&","%26",gsub('"','%22',gsub("\\", "",gsub(":","%3A",gsub("/","%2F",gsub("'","%27",gsub(" ","+",
                                                                                                                                                                                                                                                                               gsub(",","%2c",gsub("e?","e",gsub("?","%3F",Unaccent(phrase_cut),fixed=TRUE),fixed = TRUE)))))), 
                                                                                                                                                                                                                    fixed=TRUE)))))))))))), collapse = 'OR',"%5BTitle%5D%22%29"))
    ...
    (au_querry=paste0("%28",gsub("[?.]","",gsub(",","%2C",gsub("\\", "", gsub(":","%3A",gsub("/","%2F",
                                                                                             gsub(" ","+",Unaccent(phrase_cut_au))))
                                                               , fixed =TRUE))), collapse = 'OR','%5BAuthor%5D%29'))
...
ads_get_publi<-function(au_data,ti_data,position_name,pas,value_same_min_ask,value_same_min_accept,token){
  ...
  for(h in 1:inter){
    ...
    (ti_querry=paste0("%22",gsub("[]]","%5D",gsub("`",'%60',gsub("[[]","%5B",gsub("[(]","%28",gsub("[)]","%29",gsub("<","%3C",gsub(">","%3E",gsub("=","%3D",gsub('[}{$]',"",gsub("&","%26",gsub('"','%22',gsub("\\", "",gsub(":","%3A",gsub("/","%2F",gsub("'","%27",gsub(" ","%20",
                                                                                                                                                                                                                                                                          gsub(",","%2c",gsub("e?","e",gsub("?","%3F",gsub("%","%25",(tolower(Unaccent(ti_data[first:last])))),fixed=TRUE),fixed = TRUE)))))), 
                                                                                                                                                                                                               fixed=TRUE)))))))))))), collapse = 'OR','%22'))
...
get_cit_or_ref_arxiv<-function(resdt,type){ 
  ...
  if(dim(res_link_final)[1]>0) row.names(res_link_final)<-1:dim(res_link_final)[1]
...
extraction_data_api_arxiv<-function(data_pub,ti_name,au_name,pas=8,value_same_min_accept=0, value_same_min_ask=1,type="cit",source_name=""){
  ...
  if(source_name!="" && (type=="ref" || type=="all")){
    ...
    if(dim(data_wos)[1]!=0) {
      ...
      res_wos_all=axiv_get_publi(data_wos[au_name][[1]],data_wos[ti_name][[1]],pas,value_same_min_ask,value_same_min_accept)
      res_wos=res_wos_all$res
      err1=res_wos_all$error
      reject1=res_wos_all$reject
    ...
    data_rest=data_pub[data_pub[source_name]!="WOS",]
    if(dim(data_rest)[1]!=0) {
      res_rest_all=axiv_get_publi(data_rest[au_name][[1]],data_rest[ti_name][[1]],pas,value_same_min_ask,value_same_min_accept)
      res_rest=res_rest_all$res
      err2=res_rest_all[2,]$error
      reject2=res_rest_all$reject
    ...
    resdt_all=axiv_get_publi(au_data,ti_data,pas,value_same_min_ask,value_same_min_accept)
    error_querry=resdt_all$error
    reject=resdt_all$reject
    res=resdt_all$res
  ...
  if(length(res)>0){
    ...
    resdt_reject<-as.data.frame(res_reject)
    
    # colnames(resdt)<-c("CitÃ©","Titre", "Auteur","Domaine","Journal")
    # ft<-Sys.time()
...
axiv_get_publi<-function(au_data,ti_data,pas,value_same_min_ask,value_same_min_accept){
  ...
  for(h in 1:inter){# boucle principale qui parcour les données 
    ...
    (au_querry<-gsub(",",'+AND+',paste0("%28",gsub("[?]","",au_data[first:last]),"%29", collapse = '+OR+')))
    ti_querry<-paste0(ti_data[first:last], collapse = '"+OR+"')
    
    #querry<-paste0('http://export.arxiv.org/api/query?search_query=(au:%20"',au_querry,'")&start=0&max_results=2000')
    querry<-paste0('http://export.arxiv.org/api/query?search_query=au:%28',au_querry,'%29+AND+ti:%28"',ti_querry,'"%29&start=0&max_results=2000')   #%20AND%20ti:%28"',ti_querry,'"%29
    querry=gsub(" ","+",querry)
...
fit_name_position<-function(au_data,position_base,position_togo){
  au_sep=strsplit(au_data,",")
